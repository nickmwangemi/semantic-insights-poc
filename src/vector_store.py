...
class VectorStore:
    ...
    def __init__(self, use_pinecone: bool = True):
        self.use_pinecone = (
            use_pinecone and PINECONE_AVAILABLE and Config.PINECONE_API_KEY
        )
        ...
        try:
            ...
            if self.index_name not in pc.list_indexes().names():
                pc.create_index(
                    ...
                    metric="cosine",
                    spec=ServerlessSpec(
                        cloud="aws", region=Config.PINECONE_ENVIRONMENT or "us-east-1"
                    ),
                )
        ...
        try:
            ...
            if filters:
                ...
                if "urgency_level" in filters:
                    pinecone_filter["urgency_level"] = {
                        "$gte": filters["urgency_level"]
                    }
    ...
    def _matches_filters(self, item: Dict, filters: Dict) -> bool:
        ...
        if "business_focus" in filters:
            if (
                metadata.get("business_focus", "").lower()
                != filters["business_focus"].lower()
            ):
                return False
    ...
    def delete_by_id(self, embedding_id: str) -> bool:
        ...
        if self.use_pinecone:
            ...
            try:
                ...
                embeddings_data = [
                    item for item in embeddings_data if item["id"] != embedding_id
                ]
            ...
            try:
                if os.path.exists(self.local_embeddings_path):
                    ...
                    return {
                        ...
                        "dimension": (
                            len(embeddings_data[0]["embedding"])
                            if embeddings_data
                            else 0
                        ),
                    }
...
class EnhancedSemanticSearchEngine:
    ...
    def _explain_match(self, query: str, matched_item: Dict) -> str:
        ...
        if any(
            word in metadata.get("mindset_pattern", "").lower()
            for word in query_lower.split()
        ):
    ...
    def search_with_business_filter(
        self, query: str, business_types: List[str], top_k: int = 5
    ) -> List[Dict]:
    ...
    def search_by_urgency(
        self, query: str, min_urgency: int = 3, top_k: int = 5
    ) -> List[Dict]:
    ...
    def get_similar_participants(
        self, participant_name: str, top_k: int = 5
    ) -> List[Dict]:
        ...
        search_query = (
            f"{metadata.get('primary_goal', '')} {metadata.get('main_blocker', '')}"
        )
